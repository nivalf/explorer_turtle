...
class VisualiseMarkers:
    def __init__(self):
        ...
        for i in range(SECTORS):
            ...
            led_marker.pose.position.x = OBSTACLE_THRESHOLD * 0.75 * cos((i - 0.5) * pi / 6)  # adjust for left and right sector
            led_marker.pose.position.y = OBSTACLE_THRESHOLD * 0.75 * sin((i - 0.5) * pi / 6)
        ...
        for i in range(SECTORS + 1):  # one extra for the front line
            ...
            line_marker.id = self.led_markers[-1].id + i
            ...
            end_point = Point()
            end_point.x = OBSTACLE_THRESHOLD * cos((i - 1) * pi / 6)  # adjust for front, left, and right lines
            end_point.y = OBSTACLE_THRESHOLD * sin((i - 1) * pi / 6)
            line_marker.points.append(end_point)
            self.line_markers.append(line_marker)

        print(f'Line markers: {self.line_markers}')
        ...
        circle_marker.scale.x = 2 * OBSTACLE_THRESHOLD
        circle_marker.scale.y = 2 * OBSTACLE_THRESHOLD
        ...
        rospy.loginfo("The 'sector_viz' node is active")

    
    def shutdown_ops(self):
        # Add any shutdown actions as necessary
        rospy.logwarn("Received a shutdown request. Shutting down 'sector_viz' node...")

    def set_led_markers(self):
        obstacle_detected = self.tb3_lidar.min_distance < OBSTACLE_THRESHOLD


        if(obstacle_detected):
            print(f'Lidar min: {self.tb3_lidar.min_distance}')
            print(f'Obstacle detected at {self.tb3_lidar.closest_object_position}')
            # 1 - left sector
            # 0 - right sector
            sector_with_obstacle = 1 if self.tb3_lidar.closest_object_position < 0 else 0

            for i in range(SECTORS):
                if i==sector_with_obstacle:
