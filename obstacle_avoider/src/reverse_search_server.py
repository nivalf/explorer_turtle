#! /usr/bin/env python3
# reverse_search_action_server.py
...
class ReverseSearchActionServer():
    ...
    def __init__(self):
        ...
        self.actionserver = actionlib.SimpleActionServer(
            "reverse_search_action_server", SearchAction, self.action_server_launcher, auto_start=False)
        ...
        rospy.loginfo("The 'Reverse Search Action Server' is active...")
    ...
    def action_server_launcher(self, goal: SearchGoal):
        try:
            ...
            # DEV: currently using the fwd velocity of Search action as the reverse velocity
            # DEV: Implemente a reverse search action messages
            reverse_velocity = goal.fwd_velocity
            ...
            print(f"\n#####\n"
                f"The 'reverse_search_action_server' has been called.\n"
                f"Goal: Move backwards with {reverse_velocity:.2f} m/s velocity and stop at {approach_distance:.2f} distance from the object ...\n\n"
            ...
            # TODO: set the robot's reverse velocity (as specified in the "goal")...
            self.vel_controller.set_move_cmd(reverse_velocity)
            ...
            while self.tb3_lidar.min_distance_behind > approach_distance:
                # update LaserScan data:
                self.closest_object = self.tb3_lidar.min_distance_behind
                self.closest_object_location = self.tb3_lidar.closest_object_position
                ...
                self.result.closest_object_angle = 0.0 # DEV TODO: needs implementation
    ...
    def is_req_valid(self, goal: SearchGoal):
        ...
        if reverse_velocity > 0:
            print(f'Invalid approach velocity {reverse_velocity:.2f}m/s.')
            is_valid = False
        elif reverse_velocity < -0.26:
            print(f'Invalid approach velocity {reverse_velocity:.2f}m/s. Please choose an approach velocity between 0m/s and -0.26m/s.')
            is_valid = False
...
if __name__ == '__main__':
    rospy.init_node("reverse_search_action_server")
    ReverseSearchActionServer()
    rospy.spin()
